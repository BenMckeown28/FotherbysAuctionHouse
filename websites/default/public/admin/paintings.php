...
<!-- Query to show all the paintings where the status is set to pending -->
<?php
require '../config.php';

$results = $pdo->query('SELECT * FROM Paintings WHERE status = "pending"');
$i=1;
...
'<td> <a>'.$i.'</a></td>'.
      ...
      '<td><a>'.$row['medium_used'] .'</a></td>'.
        ...
        '<ul>'.
        '<li><td><a href="approvepainting.php?id='.$row['idPaintings'].'">Approve</a></li>'.
        '<li><a href="removepaintings.php?id='.$row['idPaintings'].'">Remove</a></li>.
<li><a href="viewpainting.php?id='.$row['idPaintings'].'">Edit</a>
