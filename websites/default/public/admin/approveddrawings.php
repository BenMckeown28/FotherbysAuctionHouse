...
<!-- Selects all records from drawings where the status is set to approved -->
<?php
require '../config.php';

$results = $pdo->query('SELECT * FROM Drawings WHERE status = "approved"');
$i=1;
...
'<td> <a>'.$i.'</a></td>'.
      ...
      '<td><a>'.$row['Drawing_medium'] .'</a></td>'.
        ...
        '<ul>'.
        '<li><td><a href="pendingdrawing.php?id='.$row['idDrawings'].'">Pending</a></li>'.
        '<li><a href="removedrawings.php?id='.$row['idDrawings'].'">Remove</a>
<li><a href="viewdrawings.php?id='.$row['idDrawings'].'">View</a></li>
<li><a href="solddrawings.php?id='.$row['idDrawings'].'">Sold</a></li>
