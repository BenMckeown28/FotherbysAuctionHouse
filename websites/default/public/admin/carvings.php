...
<!-- Shows all the records from the carving table where the status is set to pending -->
<?php
require '../config.php';

$results = $pdo->query('SELECT * FROM carvings WHERE status = "pending"');
$i=1;
...
'<td> <a>'.$i.'</a></td>'.
      ...
      '<td><a>'.$row['width_dimension'] .'</a></td>'.
        ...
        '<li><td><a href="approve.php?id='.$row['idcarvings'].'">Approve</a></li>'.
        '<li><a href="remove.php?id='.$row['idcarvings'].'">Remove</a></li>
