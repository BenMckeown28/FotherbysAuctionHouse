...
/* Selects the relevent record based on item which has been selected within the catalogue */
   $sculpture= $pdo->query('SELECT * FROM carvings WHERE idcarvings = '. $_GET['id'])->fetch()  ;
$id =$_GET['id'] ;
?>
<h2><?php echo $sculpture['lotnumber'].$sculpture['idcarvings'] ;  ?></h2>
<div class="con">
...
/*If the user is logged in then they can leave a commision bid on an item */
...
';
/* If the user submits a bid which is lower than the estimated price then it wont be accepted, otherwise it will be inserted INTO
the commission_bids table */
